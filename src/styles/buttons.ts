 ...
 * `BTN_PRIMARY_MD`  → in-row CTAs ("Add to cart", "View cart")
 * `BTN_PRIMARY_SM`  → tight pills inside cards ("Open in Maps")
 * `BTN_DARK_MD`     → dark variant for the Instagram pill on Home
 * `BTN_SUCCESS_MD`  → green moss variant for "In cart" state
 */

const BASE =
  "inline-flex items-center justify-center rounded-full border-2 font-semibold shadow-lg transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:translate-y-0 disabled:shadow-none";
...
export const BTN_PRIMARY_MD = `${BASE} ${TERRACOTTA_FILL} ${DISABLED} px-5 py-2 text-sm`;
export const BTN_PRIMARY_SM = `${BASE} ${TERRACOTTA_FILL} ${DISABLED} px-4 py-1.5 text-xs`;
export const BTN_DARK_MD = `${BASE} ${DARK_FILL} px-5 py-2.5 text-sm gap-2`;
export const BTN_SUCCESS_MD = `${BASE} ${SUCCESS_FILL} px-5 py-2 text-sm`;
